---
paths: challenges/**/README.md
---

# Writeup Format Specification

This document defines the standard format for CTF writeups in this repository.
Each challenge writeup is stored as `README.md` within its challenge directory.

## How to Generate

Use the `writeup-generator` skill or `/writeup` command.
Full format specification: `.claude/skills/writeup/format.md`
...
├── dist/          # Distributed challenge files (ignored)
│   ├── challenge.tar.gz   # Original archive
│   └── challenge/         # Extracted files
│       ├── binary
│       └── source.c
└── work/          # Working directory (ignored)
    ├── notes.md   # Scratch notes
    ├── debug.py   # Debug scripts
    └── output/    # Program output, core dumps
```

### Git Tracking Policy

| Directory/File | Git Status | Purpose |
|----------------|------------|---------|
| `README.md` | Tracked | Writeup for publication |
| `solve.py` | Tracked | Clean, documented solution |
| `dist/` | Ignored | Challenge binaries, archives |
| `work/` | Ignored | Temporary files, experiments |

### Why This Structure?

- **dist/**: Keeps large binaries and archives out of git history
- **work/**: Allows messy experimentation without cluttering commits
- **solve.py**: Final, clean solution stays in repo for reference
...
- [ ] Commands can be reproduced
- [ ] No spoilers in commit messages
- [ ] Platform publication rules are followed
- [ ] Flag is redacted if required by platform
